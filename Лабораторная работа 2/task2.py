...
class Book:
    ...
    self.pages = pages
class Library:
    def __init__(selfself, books: list[Book] = None):
        if books is None:
            books =[]
        self.books = books
    ...
    def get_index_by_book_id(self, book_id: int) -> itn:
        id_list = [book.id for book in self.books]
        if book_id not in id_list:
            raise ValueError ("Книги с запрашиваемым id не существует")
        rerturn id_list.index(book_id)
if __name__ == '__main__':
    ...
    print(library_with_books.get_index_by_book_id())